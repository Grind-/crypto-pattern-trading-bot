...
def calculate_risk_params(candles: List[Dict], capital: float,
                          regime: str, signals_count_green: int) -> Dict:
    ...
    entry = candles[-1]["close"] if candles else 1.0
    sl_pct = round(1.5 * atr / entry * 100, 3)
    tp_pct = round(2.5 * atr / entry * 100, 3)
    ...
    return {
        ...
        "reasoning": f"ATR={atr:.2f}, SL=1.5xATR({sl_pct:.2f}%), TP=2.5xATR({tp_pct:.2f}%), size={size}% ({signals_count_green}/4 grün, regime={regime})",
        "blocked": blocked,
    }
